...
class News {
    ...
    public $table = "event_news";
    private $columns = array("news_id", "news_event", "news_title", "news_desc", "news_datetime_created", "news_status");
    private $column_create = array("news_event", "news_title", "news_desc", "news_status", "news_datetime_created");
    
  
    
    
    public $identifierColumn, $identifierValue, $columnName, $columnValue, $tableName, $sql, $status, $result, $total, $countResult, $id, $event, $title, $desc, $status, $createdAt;
    
    private $col_id                 = "news_id";
    private $col_event              = "news_event";
    private $col_title              = "news_title";
    private $col_desc               = "news_desc";
    private $col_datetime_created   = "news_datetime_created";
    private $col_status             = "news_status";
    ...
    public function create(){
        ...
        if(isset($this->event) && isset($this->title) && isset($this->desc) && isset($this->status)) {
            
            $event          = $this->database->real_escape_string($this->sanitizeStringV2($this->event));
            ...
            $status         = $this->database->real_escape_string(intval($this->status));
            $date           = new DateTime();
            $dateTimeToday  = $date->format("Y-m-d H:i:s");
            $imploded       = $this->implodedArray($this->column_create);
            
            $sql = "INSERT INTO ".$this->table." (".$imploded.") VALUES(".$event.", '".$title."', '".$desc."', '".$status."', '".$dateTimeToday."') ";
    ...
    public function update(){
        ...
        if(isset($this->id) && isset($this->event) && isset($this->title) && isset($this->desc) && isset($this->status)) {
            ...
            if($result) {
                ...
                if($result->num_rows > 0) {
                    ...
                    if($row = $result->fetch_object()){
                        ...
                        $this->id           = $row->news_id;
                        $this->event        = $row->news_event;
                        $this->title        = $row->news_title;
                        $this->desc         = $row->news_desc;
                        $this->status       = $row->news_status;
                        $this->createdAt    = $row->news_datetime_created;
