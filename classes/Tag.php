...
class Tag {
    ...
    public $table = "event_tag";  
    private $columns = array("tag_id", "tag_event", "tag_organiser", "tag_title", "tag_created_at");
    private $column_create = array("tag_event", "tag_organiser", "tag_title", "tag_created_at");
    
    public $identifierColumn, $identifierValue, $columnName, $columnValue, $tableName, $sql, $status, $result, $total, $countResult, $id, $event, $organiser, $title, $createdAt;
    
    private $col_id                 = "tag_id";
    private $col_event              = "tag_event";
    private $col_organiser          = "tag_organiser";
    private $col_title              = "tag_title";
    private $col_datetime_created   = "tag_created_at";
    ...
    public function create(){
        ...
        if(isset($this->event) && isset($this->organiser) && isset($this->title)) {

            $event          = $this->database->real_escape_string(intval($this->event));
            $organiser      = $this->database->real_escape_string(intval($this->organiser));
            ...
            $sql = "INSERT INTO ".$this->table." (".$imploded.") VALUES(".$event.", ".$organiser.", '".$title."', '".$dateTimeToday."') ";
            ...
            if($result) {
                ...
                if($result->num_rows > 0) {
                    ...
                    if($row = $result->fetch_object()){
                        ...
                        $this->id           = $row->tag_id;
                        $this->event        = $row->tag_event;
                        $this->organiser    = $row->tag_organiser;
                        $this->title        = $row->tag_title;
                        $this->createdAt    = $row->tag_created_at;
